...
public class Component {
    ...
    public <T extends Component> List<T> findComponents(Class<T> componentClass, WebDriver driver){
        ...
        String cssSeletor;
        try{
            cssSeletor = componentClass.getAnnotation(ComponentCssSelector.class).value();
        } catch (Exception e){
            throw new IllegalArgumentException("[ERR] The component MUST have a selector");
        }

        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector(cssSeletor)));
        List<WebElement> results = component.findElements(By.cssSelector(cssSeletor));
        ...
        try{
            ...
            throw new IllegalArgumentException("[ERR] Component MUST have constructor with params: " + Arrays.toString(params));
        }
